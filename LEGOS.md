...
*   **[`StatCounters.astro`](file:///templates/starter/src/components/StatCounters.astro)**: Trust-building stat band — large numbers ("50+ Homes Built", "15 Years Local") that count up from zero once the row scrolls into view, then hold. Dependency-free Astro + vanilla JS (a single `IntersectionObserver` per instance triggers the count-up once; respects `prefers-reduced-motion` by jumping straight to the final values). Best as a mid-page trust break right after a hero, gallery, or services section — works for any business with real numbers to show off (years in business, projects completed, rating, service area count).
    *   **Props**: all optional, falling back to `content.json`'s `home.statCounters` block: `badgeOverride`, `headingOverride`, `itemsOverride` (array of `{ value, prefix?, suffix?, label, decimals? }`), `variant` (`'dark' | 'light'`, default `'dark'`).
*   **[`VideoTestimonialWall.astro`](file:///templates/starter/src/components/VideoTestimonialWall.astro)**: Masonry wall of short client video testimonials (real `<video>` elements, not embeds — works with locally hosted clips). Desktop pointers get a muted, looping autoplay preview on hover (skipped entirely for touch devices and under `prefers-reduced-motion`); any device can tap/click a card to open a full lightbox with sound and native controls. Pure Astro + vanilla JS, no video provider or player library required. Best for service businesses with real client video testimonials — home builders, remodelers, med spas, fitness studios — anywhere a face-to-camera quote lands harder than text.
    *   **Props**: all optional, falling back to `content.json`'s `home.videoTestimonialWall` block: `badgeOverride`, `headingOverride`, `subtitleOverride`, `itemsOverride` (array of `{ videoSrc, poster, name, location?, quote? }`). Requires the client's actual video files (and poster thumbnails) to be dropped into `public/` — there's no stock/placeholder video source.
